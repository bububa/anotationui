...
<script lang="ts">
  ...
  import { getHandGuestureLabelInfo } from '@/api/handguesture/labelInfo';
  import { getHandGuestureSampleList } from '@/api/handguesture/sampleList';
  import { delHandGuestureSample } from '@/api/handguesture/delSample';

  export default defineComponent({
    name: 'HandGuestureInfo',
    components: {
      ReloadOutlined,
      Item,
    },
    ...
    setup(props) {
      ...
      async function fetch() {
        ...
        try {
          ...
          const res: RecordListResponseModel = await getHandGuestureSampleList(payload);
      ...
      async function getLabel (labelID: number) {
        try {
          const res: RecordModel = await getHandGuestureLabelInfo(labelID) 
          labelRef.value = res
        }catch{}
      }
      ...
      async function handleRemove(record: RecordModel) {
        try {
          await delHandGuestureSample(record)
          await fetch()
        }catch{}
      }
