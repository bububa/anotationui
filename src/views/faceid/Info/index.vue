...
<script lang="ts">
  ...
  import { getFaceIDLabelInfo } from '@/api/faceid/labelInfo';
  import { getFaceIDSampleList } from '@/api/faceid/sampleList';
  import { delFaceIDSample } from '@/api/faceid/delSample';
    ...
    setup(props) {
      ...
      async function fetch() {
        ...
        try {
          ...
          const res: RecordListResponseModel = await getFaceIDSampleList(payload);
      ...
      async function getLabel (labelID: number) {
        try {
          const res: RecordModel = await getFaceIDLabelInfo(labelID) 
          labelRef.value = res
        }catch{}
      }
