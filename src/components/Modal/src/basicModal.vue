<template>
  <n-modal id="basic-modal" v-bind="getBindValue" v-model:show="isModal" @close="onCloseModal">
    ...
    <template #action v-if="!$slots.action">
      <n-space>
        <n-button @click="closeModal">{{ cancelBtuText }}</n-button>
        <n-button v-if="showConfirmBtn" type="primary" :loading="subLoading" @click="handleSubmit">{{
          subBtuText
        }}</n-button>
      </n-space>
    </template>
...
<script lang="ts" setup>
  ...
  const showConfirmBtn = computed(() => {
    const { showConfirmBtn } = propsRef.value as any;
    return showConfirmBtn || props.showConfirmBtn;
  });
