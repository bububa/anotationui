<template>
  <div class="w-full">
    ...
    <!--上传图片-->
    <n-space>
      <n-alert title="instruction" type="info" v-if="helpText" class="flex w-full">
        {{ helpText }}
      </n-alert>
    </n-space>
  </div>
    ...
    setup(props, { emit }) {
      ...
      function beforeUpload({ file }) {
        ...
        if (maxSize && fileInfo.size / 1024 / 1024 >= maxSize) {
          message.error(`The maximum image size should be less than ${maxSize}M`);
          return false;
        }
        ...
        if (acceptRef.length > 0 && !checkFileType(fileInfo.type)) {
          message.error(`Only support file types: ${fileType.join(',')}`);
          return false;
        }
